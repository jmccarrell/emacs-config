...
**Rule: Claude does not write to global Emacs state without explicit Jeff approval in chat.** Anything under `~/.emacs.d/` or another non-worktree, non-git-managed Emacs location is Jeff-side state, even when a tool or package command can technically write there. That includes symlinks, installed helper scripts such as `emacs-mcp-stdio.sh`, generated caches, and other global Emacs artifacts. Claude may inspect these locations read-only when needed, but must not create, modify, remove, install, or regenerate files there unless Jeff explicitly approves that specific global-state write in the conversation. Exception: Claude may install Emacs packages into `~/.emacs.d/elpa/` when the package is fetched from a package archive already configured in Jeff's Emacs config, such as the `package-archives` named in `jeff-emacs-config.org`. Package installs from arbitrary URLs, `package-vc`, Git checkouts, or unconfigured archives still require explicit Jeff approval. Prefer writing project-tracked source or documenting the Jeff-side command to run.
