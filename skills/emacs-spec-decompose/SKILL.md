...
- **literate-emacs.d config changes** — every sub-goal gets its own feature worktree, created with `wt switch --create <feature>` from the `literate-emacs.d` checkout (then repoint the `~/.emacs.d/init.el` symlink if the sub-goal changes `init.el`); see workspace `CLAUDE.md`'s "Feature worktrees (for Claude agents)" section. I write the **branch-tracked** TASK.md inside it: its Goal is the sub-goal, its Why links to the spec. During execution, after `just verify-tangle` passes, stage `jeff-emacs-config.org` and `init.el` together and commit — they must always land in the same commit. Remove TASK.md before the branch is merged. Literate-config git details live in `literate-emacs.d/CLAUDE.md`'s "Git workflow" section.
- **Workspace-level edits** — point at workspace `CLAUDE.md`'s "Meta-doc edits" rule: a worktree in `../emacs-config.worktrees/<feature>` and a PR, same as config changes. The old fast path (direct edits in the main checkout plus a hand-pasted commit) is retired.
